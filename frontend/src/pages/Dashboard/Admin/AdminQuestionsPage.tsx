...
import { QuestionModerationPanel } from "../../../components/admin";
    ...
    <div className={`flex min-h-screen ${isDark ? "bg-slate-950" : "bg-slate-50"}`}>
      ...
      <main className={`flex-1 overflow-y-auto px-6 py-6 lg:px-10 lg:py-7 ${isDark ? "text-white" : "text-slate-900"}`}>
        <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h1 className={`text-2xl font-bold tracking-tight ${isDark ? "text-white" : "text-slate-950"}`}>Question Moderation</h1>
            <p className={`mt-1 text-sm ${isDark ? "text-gray-400" : "text-slate-500"}`}>
              Review, approve, or hide AI-generated and manual questions.
            </p>
          </div>
        ...
        <QuestionModerationPanel isDark={isDark} />
      </main>
    </div>
  );
};
