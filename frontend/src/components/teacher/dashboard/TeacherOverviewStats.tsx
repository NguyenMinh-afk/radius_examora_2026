import React from "react";

interface TeacherOverviewStatsProps {
  overview: {
    classCount: number;
    studentCount: number;
    examCount: number;
    assignmentCount: number;
    openAssignments: number;
    pendingGrades: number;
  };
  isDark?: boolean;
}

const StatCard: React.FC<{
  title: string;
  value: string | number;
  subtitle?: string;
  icon: React.ReactNode;
  color: string;
  isDark?: boolean;
}> = ({ title, value, subtitle, icon, color, isDark }) => (
  <div className={`rounded-2xl border shadow-sm p-5 flex items-start gap-4 ${
    isDark ? "bg-slate-900 border-white/10" : "bg-white border-slate-200"
  }`}>
    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${color}`}>
      {icon}
    </div>
    <div>
      <p className={`text-sm font-medium ${isDark ? "text-gray-400" : "text-slate-500"}`}>{title}</p>
      <p className={`text-2xl font-bold mt-1 ${isDark ? "text-white" : "text-slate-900"}`}>{value}</p>
      {subtitle && <p className={`text-xs mt-0.5 ${isDark ? "text-gray-500" : "text-slate-400"}`}>{subtitle}</p>}
    </div>
  </div>
);

const TeacherOverviewStats: React.FC<TeacherOverviewStatsProps> = ({ overview, isDark }) => {
  const getColorClasses = (color: string) => {
    if (isDark) {
      const darkColors: Record<string, string> = {
        "bg-blue-50": "bg-blue-500/20",
        "bg-indigo-50": "bg-indigo-500/20",
        "bg-purple-50": "bg-purple-500/20",
        "bg-teal-50": "bg-teal-500/20",
        "bg-green-50": "bg-green-500/20",
        "bg-amber-50": "bg-amber-500/20",
      };
      return darkColors[color] || color;
    }
    return color;
  };

  const getIconColor = (color: string) => {
    if (isDark) {
      const darkIconColors: Record<string, string> = {
        "text-blue-600": "text-blue-400",
        "text-indigo-600": "text-indigo-400",
        "text-purple-600": "text-purple-400",
        "text-teal-600": "text-teal-400",
        "text-green-600": "text-green-400",
        "text-amber-600": "text-amber-400",
      };
      return darkIconColors[color] || color;
    }
    return color;
  };

  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mt-6">
      <StatCard
        title="Lớp học"
        value={overview.classCount}
        icon={
          <svg className={`w-6 h-6 ${getIconColor("text-blue-600")}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
        }
        color={getColorClasses("bg-blue-50")}
        isDark={isDark}
      />
      <StatCard
        title="Sinh viên"
        value={overview.studentCount}
        icon={
          <svg className={`w-6 h-6 ${getIconColor("text-indigo-600")}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
          </svg>
        }
        color={getColorClasses("bg-indigo-50")}
        isDark={isDark}
      />
      <StatCard
        title="Đề thi"
        value={overview.examCount}
        icon={
          <svg className={`w-6 h-6 ${getIconColor("text-purple-600")}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
        }
        color={getColorClasses("bg-purple-50")}
        isDark={isDark}
      />
      <StatCard
        title="Bài đã giao"
        value={overview.assignmentCount}
        icon={
          <svg className={`w-6 h-6 ${getIconColor("text-teal-600")}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
          </svg>
        }
        color={getColorClasses("bg-teal-50")}
        isDark={isDark}
      />
      <StatCard
        title="Đang mở"
        value={overview.openAssignments}
        subtitle="bài thi"
        icon={
          <svg className={`w-6 h-6 ${getIconColor("text-green-600")}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        }
        color={getColorClasses("bg-green-50")}
        isDark={isDark}
      />
      <StatCard
        title="Chưa chấm"
        value={overview.pendingGrades}
        subtitle="bài làm"
        icon={
          <svg className={`w-6 h-6 ${getIconColor("text-amber-600")}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
          </svg>
        }
        color={getColorClasses("bg-amber-50")}
        isDark={isDark}
      />
    </div>
  );
};

export default TeacherOverviewStats;
