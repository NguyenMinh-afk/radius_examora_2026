...
class QuestionQualityEvaluator:
    ...
    def evaluate_question(
        self, question: dict[str, Any], expected_difficulty: str = "medium"
    ) -> QualityMetrics:
        """
        ...
        if metrics.explanation_quality < 0.5:
            ...
            metrics.suggestions.append(
                "Provide clearer explanation linking to the correct answer"
            )
        else:
            metrics.strengths.append("Explanation clearly justifies the answer")

        # 6. Difficulty appropriateness
        metrics.difficulty_appropriate = self._evaluate_difficulty(
            question, expected_difficulty
        )
        if metrics.difficulty_appropriate < 0.6:
            metrics.weaknesses.append(
                "Question difficulty may not match intended level"
            )
        ...
        if self.source_keywords:
            ...
            if metrics.context_relevance < 0.5:
                ...
                metrics.suggestions.append(
                    "Ensure question content relates directly to source material"
                )
        ...
        if metrics.internal_consistency < 0.6:
            ...
            metrics.suggestions.append(
                "Ensure all options follow same format and detail level"
            )
    ...
    def _evaluate_distractors(self, question: dict) -> float:
        ...
        distractors = [
            options[k] for k in ["A", "B", "C", "D"] if k != correct and k in options
        ]
        ...
        for distractor in distractors:
            ...
            if ("not" in d_lower) != ("not" in correct_answer):
                if (
                    d_lower.replace("not ", "") in correct_answer
                    or correct_answer.replace("not ", "") in d_lower
                ):
                    issues += 0.15
    ...
    def evaluate_batch(
        self, questions: list[dict], expected_difficulty: str = "medium"
    ) -> dict[str, Any]:
        """
        ...
        aggregate = {
            ...
            "schema_valid_count": sum(
                1 for m in individual_metrics if m["schema_valid"]
            ),
            "average_overall_score": (
                round(statistics.mean(valid_scores), 3) if valid_scores else 0
            ),
            "median_overall_score": (
                round(statistics.median(valid_scores), 3) if valid_scores else 0
            ),
            ...
            "std_overall_score": (
                round(statistics.stdev(valid_scores), 3) if len(valid_scores) > 1 else 0
            ),
...
def run_quality_evaluation(
    questions: list[dict], source_context: str = "", expected_difficulty: str = "medium"
) -> dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    print(
        f"Score Range: {results['aggregate']['min_overall_score']:.3f} - {results['aggregate']['max_overall_score']:.3f}"
    )
    ...
    for m in results["individual"]:
        ...
        print(
            f"    Strengths: {', '.join(m['strengths']) if m['strengths'] else 'None'}"
        )
        print(
            f"    Weaknesses: {', '.join(m['weaknesses']) if m['weaknesses'] else 'None'}"
        )
